...
class StudentModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs  # For additional arguments like quantization_config
    ):
        super().__init__()
...
**Ready for the next script? The next one is `pruning.py` which implements all pruning methods (magnitude, gradual, wanda, structured).**
