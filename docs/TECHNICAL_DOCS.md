...
StudentModel:
  ...
  └── classifier: Sequential
      ├── Linear(768 → classifier_hidden_size)
      ├── ReLU()
      ├── Dropout(0.1)
      └── Linear(classifier_hidden_size → num_labels)
```

**Note**: The `classifier_hidden_size` (default: 256) allows for a more compact classification head than the teacher, further reducing parameters.
...
**Algorithm**: Slowly increase sparsity during training using a schedule (Linear, Cubic, or Exponential).

```python
def get_target_sparsity_at_step(current_step, total_steps, target_sparsity):
...
**Pros**:
- Allows model to adapt to weight removal
- Better accuracy recovery than one-shot pruning
...
For imbalanced datasets, the framework automatically finds the optimal classification threshold to maximize the Macro F1 score.

```python
def find_best_threshold(logits, labels, thresholds=np.linspace(0.1, 0.9, 81)):
    best_f1 = 0
    best_threshold = 0.5
...
**Why it matters**: Default thresholds (0.5) are often suboptimal for rare classes. Exploration can boost F1 by 2-5%.
...
graph TD
    ...
    main --> distill[src/distillation.py]
    main --> prune[src/pruning.py]
    main --> quant[src/quantization.py]
    main --> eval[src/evaluation.py]
...
#### Pipeline Control
```bash
--pipeline {baseline,baseline_kd,baseline_prune,baseline_quant,baseline_kd_prune_quant,...}
--dataset_path PATH
--author_name STR
