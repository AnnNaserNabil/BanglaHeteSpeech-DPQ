...
def get_pruner(
    ...
    **kwargs
    ...
    This is what main.py calls to get a pruner.
    """
    if method == 'magnitude':
...
for sparsity in 0.3 0.4 0.5 0.6 0.7; do
    python main.py --pipeline prune_only \
...
**Ready for the next script? The next one is `quantization.py` which implements all quantization methods (dynamic, static, QAT, FP16, INT4).**
