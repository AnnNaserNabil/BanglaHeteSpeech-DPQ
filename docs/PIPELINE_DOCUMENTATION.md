...
#### Purpose
Transfer knowledge from large teacher (110M params) to smaller student (e.g., DistilBERT, 66M params).

> **Note**: The student model is chosen for its efficient architecture (fewer layers) while maintaining high capacity.
...
#### Pruning Strategy: Magnitude, Wanda, or Gradual
...
pruner = PruningManager(
    ...
    method='magnitude',   # or 'wanda', 'gradual'
    prune_layers='all'
)
```

**Gradual Pruning (Optional):**
If enabled, sparsity increases slowly over training steps (e.g., using a cubic schedule) to allow better accuracy recovery.

**Wanda Pruning (Optional):**
Uses both weights and activations to determine importance. Requires a small calibration set.
...
#### Quantization Methods: FP16, INT8, or INT4
...
**INT4/NF4 (GPU):**
Uses `bitsandbytes` to quantize weights to 4 bits while keeping computation in FP16.
```python
...
if explore_thresholds:
    best_threshold, best_f1 = find_optimal_threshold(logits, labels)
    # Re-calculate metrics with best_threshold
```
