...
# Removed hardcoded LABEL_COLUMNS and LABEL_PRIORITY
    ...
    def _get_predictions(self, model, dataloader, device, use_student_input_ids=False):
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="   Predicting"):
                ...
                # Ensure logits is 2D [batch, num_labels]
                if logits.dim() == 1:
...
def calculate_aggregate_metrics(all_fold_metrics: List[List[CompressionStageMetrics]]) -> List[Dict]:
    ...
    all_fold_metrics: List of lists (one list of stages per fold)
    """
    ...
    for stage_idx in range(num_stages):
        ...
        # Select numeric columns only for mean/std
