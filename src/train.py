...
def run_kfold_training(config, comments, labels, tokenizer, device):
    ...
    with mlflow.start_run(run_name=f"{config.author_name}_batch{config.batch}_lr{config.lr}_epochs{config.epochs}", nested=True):
        ...
        summary_data = {
            'Model': [config.model_path] * config.num_folds,
            'Batch Size': [config.batch] * config.num_folds,
            'Learning Rate': [config.lr] * config.num_folds,
            'Epochs': [config.epochs] * config.num_folds,
            ...
            'total_parameters': [model_metrics['total_parameters']] * config.num_folds,
            'trainable_parameters': [model_metrics['trainable_parameters']] * config.num_folds
        }
        ...
        best_model_path = os.path.join(output_dir, f'best_model_{model_name_safe}_batch{config.batch}_lr{config.lr}_epochs{config.epochs}_{timestamp}.pt')
