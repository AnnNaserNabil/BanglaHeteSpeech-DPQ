...
class PruningManager:
    ...
    def apply_magnitude_pruning(self, sparsity: Optional[float] = None):
        ...
        if self.global_pruning:
            ...
            # Ensure parameters are contiguous to avoid view errors in global_unstructured
    ...
    def make_pruning_permanent(self):
        ...
        """
        print(f"   Debug: Checking {len(self.prunable_modules)} prunable modules for permanent pruning...")
        count_removed = 0
        for module, param_name in self.prunable_modules:
            has_mask = hasattr(module, param_name + "_mask")
            is_pruned_check = prune.is_pruned(module)
            has_hooks = len(module._forward_pre_hooks) > 0
            ...
            if is_pruned_check:
                ...
                if has_hooks:
                    print(f"   ⚠️  Module has hooks but is_pruned=False! Mask: {has_mask}")
                    # Force remove hooks if they exist but mask is missing
                    module._forward_pre_hooks.clear()
                    print("   ⚠️  Forced hook removal.")
        
        print(f"   ✅ Pruning made permanent (masks applied to weights). Removed {count_removed} masks.")
...
def get_pruner(method: str, model: nn.Module, config) -> PruningManager:
    ...
    if method == 'magnitude':
        return PruningManager(
            ...
            global_pruning=False # Debugging: Disable global pruning
        )
